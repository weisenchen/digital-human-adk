...
export default function WorkReportMode({
  ...
  // Auto-hide chat when not in Q&A phase
  useEffect(() => {
    if (phase === 'presentation') setShowChat(false);
  ...
  return (
    <div className="fixed inset-0 bg-white z-40 flex flex-col">
      ...
      {/* Main content: fullscreen slide viewer + floating chat */}
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col bg-gradient-to-b from-white to-gray-50">
          ...
          <div className="shrink-0 border-t border-gray-200 px-4 py-3 bg-white">
            <div className="flex items-center justify-between">
              {/* Left: slide position dots */}
              <div className="flex items-center gap-1.5">
                {slides.map((_, i) => (
                  <button
                    key={i}
                    onClick={() => { if (!isWaiting) { stopReading(); setCurrentSlide(i); } }}
              ...
              {/* Right: toggle chat + auto + read/stop */}
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setShowChat(!showChat)}
                  className={`flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-lg font-medium transition-colors ${
                    showChat ? 'bg-gray-200 text-gray-700' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                  }`}
        ...
        {showChat && (
          <div className="w-[400px] border-l border-gray-200 flex flex-col bg-white shadow-2xl">
            {/* Phase banner */}
            {phase === 'cto-qa' && !isReading && !isWaiting && (
              <div className="shrink-0 px-4 py-2 bg-purple-50 border-b border-purple-100 flex items-center justify-between">
                <span className="text-xs text-purple-700 font-medium">
                  💬 {config.reportToRole} Q&A — Ask questions
                </span>
                <button
            ...
            {phase === 'ai-questions' && !isReading && !isWaiting && (
              <div className="shrink-0 px-4 py-2 bg-amber-50 border-b border-amber-100">
                <span className="text-xs text-amber-700 font-medium">
                  ❓ AI is asking questions — please respond
                </span>
              </div>
            )}
            {phase === 'ended' && (
              ...
              <div className="shrink-0 border-t border-gray-200 px-4 py-3 bg-white">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder={phase === 'ai-questions' ? 'Answer the AI question...' : 'Ask a question...'}
                    disabled={isWaiting}
